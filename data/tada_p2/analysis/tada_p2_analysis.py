...
path = r"C:\Users\the1k\source\repos\PythonApplication1\catkin_ws_remote\data\tada_p2"
...
if step==0:
    ...
    for topic in topics:
        print('rostopic echo -b {}\{} -p /{} > {}.csv'.format(path, files[0],topic,topic))
    #    # Execute the command and retrieve the output
        subprocess.run('rostopic echo -b {}\{} -p /{} > {}.csv'.format(path,files[0],topic,topic), capture_output=True, text=True, shell=True)
...
# read all csv files, store them in dictionaries, find the regions of time where TADA angle is non-neutral, and plot the topics for each TADA angle
    ...
    # Read europa data
    data = pd.read_csv('europa_topic.csv')
    ...
    # read imu data
    data1 = pd.read_csv('sensing_topic.csv')
    ...
    data2 = pd.read_csv('motor_command.csv')
    ...
    data3 = pd.read_csv('xsens_joint_angle.csv')
    ...
    data4 = pd.read_csv('xsens_com.csv')
    ...
    data5 = pd.read_csv('linear_moments.csv')
    ...
    with open('region_all_med.pickle', 'wb') as handle: # region_motor_all, region_angle_accurray
    ...
    with open('region_all.pickle', 'rb') as handle: #region_all_mini
    ...
    for x, region in enumerate(regions):
        ...
        chosen_moment = mx
        if chosen_moment == mx: chosen_moment_label = 'Frontal Moment'
        elif chosen_moment == my: chosen_moment_label = 'Sagittal Moment'
