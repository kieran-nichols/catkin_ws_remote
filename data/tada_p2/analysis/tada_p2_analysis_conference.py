...
figure2 = go.Figure()
figure3 = go.Figure()
figure4 = make_subplots(rows=1, cols=2, shared_xaxes=True, vertical_spacing=0.15, horizontal_spacing=.1, subplot_titles=("Peak Plantarflexor Pylon Moments", "Pylon Plantarflexor Impulses"))
figure4.update_annotations(font_size=30)
figure5 = go.Figure()
figure6 = go.Figure()
figure7 = go.Figure()
...
# read all csv files, store them in dictionaries, find the regions of time where TADA angle is non-neutral, and plot the topics for each TADA angle
    ...
    figure4.update_layout(xaxis_title='IV', yaxis_title='PF', showlegend=False, font_size=16)
    ...
    figure10.update_layout(title=f'Peaks and Impulses for Sagittal Moments for various Ankle Angles', xaxis_title='PF/IV angles', yaxis_title='Magnitude', showlegend=True, font_size=20)
    figure10.show()
    ...
    for speed, chosen_file in enumerate(chosen_files):
        ...
        for x, region in enumerate(regions):
            ...
            if name in chosen_angles:
                ...
                if name == 'PF = 10.0, EV = 0.0':
                    ...
                    figure7.add_trace(go.Scatter(x=[z - time_m[first_ind] for z in time_m[first_ind:second_ind]], y=chosen_moment[first_ind:second_ind], mode='lines', line_color=color_rgb[2-speed], name=name, legendgroup=name, fill='toself', opacity=0.35,showlegend=True))
        ...
        figure7.update_layout(title=f'Plantarflexor Moment vs Time for (PF, IV) of (10, 0)', xaxis_title='Time (s)', yaxis_title=f'Plantarflexor Moment (Nm/kg)', legend_title='Grouped by Speed', font_size = 25)
        figure8.update_layout(title=f'Hip and Knee sagittal angles for (PF, IV) of (10, 0)', xaxis2_title='Percentage of stride (0 to 100%, heel strike to heel strike)', yaxis1_title=f'Hip Flexion Angle (deg)', yaxis2_title=f'Knee Flexion Angle (deg)', legend_title='Grouped by Speed', font_size=17)
