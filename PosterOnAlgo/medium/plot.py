...
for column in europa.columns[3:4]:
    plt.plot(europa[column], label="Pylon Force\n(N)\n", linewidth=5.0, zorder=-1)
...
for column in imu.columns[6:7]:
    plt.plot( imu[column]*-10, label="10x Angular\n Velocity\nin Z\n(deg/s)\n", linewidth=5.0, zorder=-1)
...
#for column in imu.columns[1:2]:
    ...
    #plt.plot(imu[column]*-1, label="Angular Velocity\n in Z (deg/s)")
    #plt.plot(imu[imu.columns[9]], imu[column]*98, label="10x Acceleration pointing up\n in X (deg/s)")
