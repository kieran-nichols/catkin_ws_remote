...
def xsens_com_callback(data):
    ...
    #global xsens_com
    #print(data.data)
    #shared_dict = {'xsens_com': data.data}
...
def main():
    ...
    rospy.Subscriber('xsens_com', Float32MultiArray, xsens_com_callback, (shared_dict))
    rospy.Subscriber('xsens_joint_angle', Float32MultiArray, xsens_joint_angle_callback, (shared_dict))
    rospy.Subscriber('brain', numpy_msg(Floats), brain_callback, (shared_dict))
    rospy.Subscriber('sensing_topic', IMUDataMsg, imu_callback, (shared_dict))
    rospy.Subscriber('europa_topic', EuropaMsg, europa_callback, (shared_dict))
    rospy.Subscriber('chatter_control', numpy_msg(Floats), gui_callback, (shared_dict))
    rospy.Subscriber('chatter', numpy_msg(Floats), talker_callback, (shared_dict))
    rospy.Subscriber('notes', String, notes_callback, (shared_dict))
    rospy.Subscriber('gui_topic', String, gui_cmd_callback, (shared_dict))
    data_save()
    #rospy.Subscriber('processed_data', numpy_msg(Floats), processed_data_callback, (shared_dict))
